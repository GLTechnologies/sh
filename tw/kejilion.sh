...
edit_app_description() {
	...
	(
		...
		exec 9>"$lock_file"
		if ! flock -n 9; then
			echo "該應用程式描述正在被其他會話編輯，請稍後再試"
			return 1
		fi

		echo -e "${gl_huang}請輸入${docker_name}的應用描述（最多 10 行，${EDIT_TIMEOUT}秒內未儲存將自動退出）${gl_bai}"
		...
		# ===== 啟動編輯器 =====
		if command -v nano >/dev/null 2>&1; then
			echo "編輯器提示：Ctrl+O 儲存，Ctrl+X 退出"
			sleep 1
			nano "$tmp_file"
		else
			vi "$tmp_file"
	  ...
	  113|firefox)
		...
		115|BTCPay)
			...
			docker_app_install() {
				...
				read -e -p "輸入應用解析後的網域名稱(example.com):" app_domain
				install git
...
cert_manage() {
	...
	while true; do
		...
		case $sub_choice in
			1) 
				read -p "請輸入解析後的網域名稱（exmple.com）:" domain
			...
			4)
                ...
                # 判斷 nginx 目錄是否存在
                if [ ! -d "/etc/nginx" ]; then
                    echo "❌ 未偵測到 /etc/nginx，請先安裝 Nginx"
                    break_end
					continue
                fi
				...
				read -e -p "請輸入要建立的目錄名 [預設:$default_dir]: " dirname
				dirname=${dirname:-$default_dir}
				mkdir -p "$dirname" && echo "目錄已建立：$dirname" || echo "創建失敗"

				cd "$dirname" 2>/dev/null || echo "無法進入目錄"
				echo "📂 目前已有證書清單："
				ls "$dirname"

				read -e -p "請輸入要編輯的檔名（cert.pem）:" filename
				install nano
				nano "$filename"
				read -e -p "請輸入要編輯的檔名（key.pem）:" filename
				nano "$filename"
				chmod 600 "${filename}"
...
echo -e "${gl_kjlan}15.  ${gl_bai}伺服器叢集控制"
