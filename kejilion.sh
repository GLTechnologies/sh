...
UserLicenseAgreement() {
	clear
	echo -e "${gl_kjlan}欢迎使用GL科技脚本工具箱${gl_bai}"
...
syncRTC() {
	...
	if [ "$abs_diff" -gt "$Time_threshold" ]; then
		echo "系统 时间: $(date -d "@${sys_ts}" "+%Y-%m-%d %H:%M:%S")"
		echo "RTC 时间: $(date -d "@${rtc_ts}" "+%Y-%m-%d %H:%M:%S")"

    	read -p "时间相差 ${abs_diff} s，是否将系统时间同步为 RTC 时间？(y/n): " yn
        case $yn in
            [Yy] )
				echo "正在同步系统时间..."
...
echo "------------------------"
echo -e "${gl_kjlan}脚本官网: ${gl_bai}https://kejilion.sh            ${gl_kjlan}GitHub地址: ${gl_bai}https://github.com/GLTechnologies/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "更新日志"
	echo "------------------------"
	echo "全部日志: ${gh_proxy}raw.githubusercontent.com/GLTechnologies/sh/main/kejilion_sh_log.txt"
	echo "------------------------"
...
echo -e "${gl_kjlan}14.  ${gl_bai}服务管理"
echo -e "${gl_kjlan}15.  ${gl_bai}服务器集群控制"
echo -e "${gl_kjlan}16.  ${gl_bai}广告专栏"
echo -e "${gl_kjlan}17.  ${gl_bai}游戏开服脚本合集"
echo -e "${gl_kjlan}------------------------${gl_bai}"
