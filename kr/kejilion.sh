...
UserLicenseAgreement() {
	clear
	echo -e "${gl_kjlan}GL 기술 스크립트 도구 상자에 오신 것을 환영합니다${gl_bai}"
	echo "스크립트를 처음 사용하는 경우 사용자 라이센스 계약을 읽고 동의하십시오."
	echo "사용자 라이센스 계약: https://"
	echo -e "----------------------"
...
syncRTC() {
	...
	if [ "$abs_diff" -gt "$Time_threshold" ]; then
		echo "시스템 시간: $(date -d"@${sys_ts}" "+%Y-%m-%d %H:%M:%S")"
		echo "RTC 시간: $(date -d"@${rtc_ts}" "+%Y-%m-%d %H:%M:%S")"

    	read -p "시차${abs_diff}s, 시스템 시간을 RTC 시간에 동기화하시겠습니까? (예/아니요):" yn
        case $yn in
            [Yy] )
				echo "시스템 시간을 동기화하는 중..."
				rtc_ts=$(date -d "$(timedatectl | grep "RTC time" | awk -F': ' '{print $2}')" +%s)
                
				if sudo -n date -s "@$rtc_ts" >/dev/null 2>&1; then
					echo -e "${gl_lv}동기화가 완료되었습니다!${gl_bai}"
				else
					if sudo date -s "@$rtc_ts"; then
						echo -e "${gl_lv}동기화가 완료되었습니다!${gl_bai}"
					else
						echo -e "${gl_hong}동기화에 실패했습니다!${gl_bai}"
					fi
				fi
				sleep 1
                ;;
            * )
                echo -e "${gl_hong}동기화가 취소되었습니다.${gl_bai}"
				sleep 1
                ;;
        esac
	fi
...
edit_app_description() {
	...
	(
		...
		exec 9>"$lock_file"
		if ! flock -n 9; then
			echo "다른 세션에서 앱 설명을 수정하고 있습니다. 나중에 다시 시도해 주세요."
			return 1
		fi

		echo -e "${gl_huang}입력해주세요${docker_name}애플리케이션 설명(최대 10줄,${EDIT_TIMEOUT}몇 초 내에 저장하지 않으면 자동으로 종료됩니다)${gl_bai}"
		...
		if ! command -v nano >/dev/null 2>&1; then
			echo "나노가 감지되지 않습니다. 설치 중..."
			install nano >/dev/null 2>&1
		fi

		# ===== 시작 시간 초과 감시 =====
		(
			# ===== 상속을 방지하려면 잠금 FD를 끄세요 =====
			exec 9>&-

			sleep "$EDIT_TIMEOUT"
			echo "수정 시간이 초과되어 자동으로 종료되었습니다."
		...
		# ===== 편집기 실행 =====
		if command -v nano >/dev/null 2>&1; then
			echo "편집기 프롬프트: 저장하려면 Ctrl+O, 종료하려면 Ctrl+X"
			sleep 1
			nano "$tmp_file"
		else
			vi "$tmp_file"
		...
		echo "앱 설명이 저장되었습니다(${lines}/10줄)"
		# ===== 잠금 해제(기능이 종료되면 FD 9가 자동으로 해제됩니다) =====
	)
}
...
show_app_description() {
	...
	echo ""
	echo "【응용 프로그램 설명】${current_lines}/${max_total}"
	head -n "$max_lines" "$desc_file"

	# max_lines를 초과하면 프롬프트만 표시되고 출력되지 않습니다...
	if [ "$current_lines" -gt "$max_lines" ]; then
		echo "(자세한 내용은 "애플리케이션 설명"을 참조하세요)"
	fi
}
	...
	read -e -p "선택사항을 입력하세요:" choice
	 case $choice in
		...
		8)
			...
			;;
		9)
			send_stats "애플리케이션 설명${docker_name}"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}113. ${color113}파이어폭스 브라우저${gl_kjlan}114. ${color114}Xboard 노드 관리 패널"
	  echo -e "${gl_kjlan}115. ${color115}BTCPay 가상화폐 결제 플랫폼"
	  echo -e "${gl_kjlan}-------------------------"
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  113|firefox)
		...
		114|xboard)
			local app_id="114"
			local app_name="xboard 노드 관리 패널"
			local app_text="Laravel 11을 기반으로 구축된 현대적인 노드 패널 시스템으로 간단하고 효율적인 사용자 경험을 제공하는 데 중점을 두고 있습니다."
			local app_url="공식 웹사이트: https://github.com/cedar2025/Xboard"
		...
		115|BTCPay)
			local app_id="115"
			local app_name="BTCPay 결제 플랫폼"
			local app_text="자체 호스팅 오픈 소스 비트코인 ​​결제 프로세서입니다."
			local app_url="공식 홈페이지: https://github.com/btcpayserver/btcpayserver-docker"
			...
			docker_app_install() {
				...
				read -e -p "애플리케이션에서 확인된 도메인 이름을 입력하세요(example.com):" app_domain
				install git
...
linux_service() {
	while true; do
		...
		echo -e "서비스 관리"
		echo -e "${gl_kjlan}------------------------${gl_bai}"
		echo -e "${gl_kjlan}1.  ${gl_bai}인증서 관리"
		echo -e "${gl_kjlan}------------------------${gl_bai}"
		echo -e "${gl_kjlan}0.  ${gl_bai}이전 메뉴로 돌아가기"
		echo -e "${gl_kjlan}------------------------${gl_bai}"
		read -e -p "선택사항을 입력하세요:" sub_choice
		case $sub_choice in
			1)
...
cert_manage() {
	...
	while true; do
		...
		echo -e "${gl_kjlan}1. ${gl_bai}인증서 신청${gl_kjlan}2. ${gl_bai}인증서 삭제"
		echo -e "${gl_kjlan}3. ${gl_bai}인증서 자동 갱신${gl_kjlan}4. ${gl_bai}수동으로 인증서 업데이트"
		echo -e "${gl_kjlan}5. ${gl_bai}인증서 유효기간을 확인하세요"
		echo -e "${gl_kjlan}------------------------${gl_bai}"
		echo -e "${gl_kjlan}0. ${gl_bai}이전 메뉴로 돌아가기"
		echo -e "${gl_kjlan}------------------------${gl_bai}"
		read -e -p "선택사항을 입력하세요:" sub_choice
		...
		case $sub_choice in
			1) 
				read -p "확인된 도메인 이름(exmpl.com)을 입력하세요." domain
				...
				;;
			2)
				echo "인증서 삭제 기능은 아직 구현되지 않았습니다."
                break_end
				;;
			3)
				echo "Let's Encrypt 인증서 업데이트 중..."
			...
			4)
                ...
                # nginx 디렉터리가 존재하는지 확인
                if [ ! -d "/etc/nginx" ]; then
                    echo "❌ /etc/nginx가 감지되지 않습니다. 먼저 Nginx를 설치하세요."
                    break_end
					continue
                fi
				...
				read -e -p "생성할 디렉터리 이름을 입력하세요. [기본값:$default_dir]: " dirname
				dirname=${dirname:-$default_dir}
				mkdir -p "$dirname" && echo "생성된 디렉터리:$dirname" || echo "생성 실패"

				cd "$dirname" 2>/dev/null || echo "디렉토리에 들어갈 수 없습니다"
				echo "📂 현재 인증서 목록이 있습니다:"
				ls "$dirname"

				read -e -p "편집할 파일 이름(cert.pem)을 입력하십시오:" filename
				install nano
				nano "$filename"
				read -e -p "편집할 파일 이름(key.pem)을 입력하십시오:" filename
				nano "$filename"
				chmod 600 "${filename}"
...
echo "------------------------"
echo -e "${gl_kjlan}스크립트 공식 웹사이트:${gl_bai}https://kejilion.sh            ${gl_kjlan}GitHub 주소:${gl_bai}https://github.com/GLTechnologies/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "변경 로그"
	echo "------------------------"
	echo "모든 로그:${gh_proxy}raw.githubusercontent.com/GLTechnologies/sh/main/kejilion_sh_log.txt"
	echo "------------------------"
...
echo "╠╩╗║╣  ║║║  ║║ ║║║║ ╚═╗╠═╣"
echo "╩ ╩╚═╝╚╝╩╩═╝╩╚═╝╝╚╝o╚═╝╩ ╩"
echo -e "GL 기술 스크립트 도구 상자 v$sh_v"
...
echo -e "${gl_kjlan}14.  ${gl_bai}서비스 관리"
echo -e "${gl_kjlan}15.  ${gl_bai}서버 클러스터 제어"
echo -e "${gl_kjlan}16.  ${gl_bai}광고 칼럼"
echo -e "${gl_kjlan}17.  ${gl_bai}게임 서버 오프닝 스크립트 모음"
echo -e "${gl_kjlan}------------------------${gl_bai}"
